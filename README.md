...
- ✅ **학습 진행률 체크리스트** — 자료별로 완료 체크 시 실시간으로 저장되고, 난이도별/전체 진행률이 상단에 표시됩니다.
- 🔍 **Observation 상세보기** — 각 검색 단계에서 실제로 평가된 결과(제목/스니펫/채택-제외 여부/판단 근거)를 펼쳐볼 수 있어, "왜 이 자료가 걸러졌는지"까지 투명하게 보입니다.
...
- 사용자 로그인/인증 (원래 스펙에서도 스트레치 목표로 남겨둔 부분, 의도적으로 미구현)
