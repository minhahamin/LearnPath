...
- 사용자가 배포된 데모를 보고 지적한 CSS 버그 2건 수정: (1) `body`/`#root`에 `height:100%`를 쓰면
  콘텐츠가 한 화면보다 길 때 배경이 거기서 잘려 스크롤 시 흰 여백이 보이던 문제 → `min-height`로 교체.
  (2) 구름이 퍼센트 기반 `radial-gradient` 6개라 화면이 넓으면 원들이 뚝뚝 떨어져 보이던 문제 →
  타일링되는 SVG 뭉게구름 패턴 2겹으로 교체. Railway 프론트엔드 재배포 완료.
